"""Reject macOS release binaries that require an OS newer than macOS 12."""
...
def check_target(load_commands):
    ...
    for block in re.split(r"(?m)^Load command \d+\s*$", load_commands)[1:]:
        ...
        if version > MAXIMUM:
            raise ValueError(f"macOS {match[1]} exceeds supported minimum 12.0")
...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("usage: check_macos_target.py BINARY")
    try:
